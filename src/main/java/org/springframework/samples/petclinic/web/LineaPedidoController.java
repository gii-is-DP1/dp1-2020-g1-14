...
public class LineaPedidoController {
	...
	*/
	...
	/*
	@GetMapping(path="delete/{lineaPedidoId}")
	public String borrarProveedor(@PathVariable("lineaPedidoId") int lineaPedidoId, ModelMap modelMap) {
	String view="lineaPedidos/listadoLineaPedidos";
	Optional<LineaPedido> lineaPedido=lineaPedidoService.findLineaPedidoById(lineaPedidoId);
	
	if(lineaPedido.isPresent()) {
		lineaPedidoService.delete(lineaPedido.get());
		modelMap.addAttribute("message","Event succesfully deleted!");
		view=listadoLineaPedidos(modelMap);
		
		log.info("LineaPedido eliminada con éxito");
	}else {
		modelMap.addAttribute("message","Event not found!");
		view=listadoLineaPedidos(modelMap);
		
		log.error("No se ha encontrado la lineaPedido para eliminar");
	}
	return view;
	}
	
	@GetMapping(path = "/{lineaPedidoId}/edit")
	public String initUpdateForm(@PathVariable("lineaPedidoId") int lineaPedidoId, ModelMap model) {
		LineaPedido lineaPedido = this.lineaPedidoService.findLineaPedidoById(lineaPedidoId).get();
	...
	@PostMapping(value="/{lineaPedidoId}/edit")
	public String processUpdateProductoForm(@Valid LineaPedido lineaPedido, BindingResult result, @PathVariable("lineaPedidoId" ) int lineaPedidoId) {
		if (result.hasErrors()) {
			...
			this.lineaPedidoService.save(lineaPedido);
			
			
			log.info("LineaPedido editada satisfactoriamente");
		}
